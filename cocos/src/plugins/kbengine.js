...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.disconnect = function()
	{
		...
		{  
			if(KBEngine.app.socket != null)
			{
				...
				sock.close();
				// if(KBEngine.app.currserver != "loginapp"){
				// 	h1global.globalUIMgr.info_ui.show_by_info("连接关闭，请检查网络！", cc.size(600, 400), function(){if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){h1global.runScene(new LoginScene());}else{window.location.href = switches.h5entrylink;}})
				// }
			}
		}
		catch(e)
	...
	{  
		cc.log('[onclose] connect close:' + KBEngine.app.currserver);
		KBEngine.Event.fire("onDisableConnect");
		if(KBEngine.app.currserver != "loginapp") {
			KBEngine.app.reset();
		}
